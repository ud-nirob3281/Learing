...
async function fu() {
  // return 'Hello';
  return Promise.resolve('Hello'); //Same
}

//console.log(fu());

//fu().then(res => console.log(res)); //Result

//*We Cant use await on global label
/* let result = await fu();
console.log(result); */
...
//1
//errorPromise.catch(va => console.error(va));
//2
async function handelErrorFu() {
  try {
    await errorPromise;
  } catch (err) {
    // console.error(err);
